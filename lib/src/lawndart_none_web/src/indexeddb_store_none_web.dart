...
class IndexedDbStore extends Store {
  final _idbidbFactory = idb.getIdbFactorySqflite(databaseFactoryFfi);
  ...
  Future<void> _open() async {
    ...
    var db = await _idbidbFactory.open(dbName);

    if (!db.objectStoreNames.contains(storeName)) {
      db.close();
      //print('Attempting upgrading $storeName from ${db.version}');
      db = await _idbidbFactory.open(dbName, version: db.version + 1,
